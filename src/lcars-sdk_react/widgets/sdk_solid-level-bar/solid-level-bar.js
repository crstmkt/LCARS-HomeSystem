...
LCARS.element.sdk.solidLevelBar.prototype = {
  ...
  create: function (oDef) {
    var element = $(
      '<div id="' + this.data.id + '" class="sdk solid-level-bar"></div>'
    );
        ...
        if (object.data.version === "vertical") {
          ...
          if (value < transformMin) {
            if (object.data.reverse === true) {
              object.elements.wrapper.dom.css(
                "transform",
                "translateY(-" + (100 - transformMin) + "%)"
              );
            } else {
              object.elements.wrapper.dom.css(
                "transform",
            ...
            if (object.data.reverse === true) {
              object.elements.wrapper.dom.css(
                "transform",
                "translateY(-" + (100 - object.data.level) + "%)"
              );
            } else {
              object.elements.wrapper.dom.css(
                "transform",
          ...
          if (value < transformMin) {
            ...
            if (object.data.reverse === true) {
              object.elements.wrapper.dom.css(
                "transform",
                "translateX(" + (100 - object.data.level) + "%)"
              );
            } else {
              object.elements.wrapper.dom.css(
                "transform",
