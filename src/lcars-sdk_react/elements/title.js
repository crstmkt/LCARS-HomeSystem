...
LCARS.element.title = function (oDef) {
  //Begin Required
  this.data = {
    type: "title",
    id: oDef.id || "titleSID" + Math.random().toString(36).substr(2, 9),
  };
...
LCARS.element.title.prototype = {
  //Required.  Create DOM element and base class
  create: function (oDef) {
    if (oDef.href !== undefined) {
      var element = $('<a id="' + this.data.id + '" class="title"></a>');
    } else {
      var element = $('<div id="' + this.data.id + '" class="title"></div>');
    }
    return element;
  },
