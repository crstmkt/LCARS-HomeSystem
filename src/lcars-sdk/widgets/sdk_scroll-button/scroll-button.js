...
LCARS.element.sdk.scrollButton.prototype = {
  ...
  create: function (oDef) {
    var element = $(
      '<div id="' + this.data.id + '" class="sdk scroll-button"></div>'
    );
    if (!oDef.version) {
      oDef.version = this.data.version;
    }
    if (oDef.version === "vertical") {
      this.elements.up = LCARS.create({
        type: "button",
        class: { "scroll-up": true },
        tap: this.scrollUp,
        label: "▲",
        attachedTo: this.data.id,
      });
      this.elements.down = LCARS.create({
        type: "button",
        class: { "scroll-down": true },
        tap: this.scrollDown,
        altLabel: "▼",
        attachedTo: this.data.id,
      });
    ...
    if (oDef.version === "horizontal") {
      this.elements.left = LCARS.create({
        type: "button",
        class: { "scroll-left": true },
        tap: this.scrollLeft,
        label: "◀",
        attachedTo: this.data.id,
      });
      this.elements.right = LCARS.create({
        type: "button",
        class: { "scroll-right": true },
        tap: this.scrollRight,
        label: "▶",
        attachedTo: this.data.id,
      });
  ...
  scrollUp: function () {
    ...
    var scrollAmount = 60;
    if (typeof object.data.amount === "number") {
      scrollAmount = object.data.amount;
    }
