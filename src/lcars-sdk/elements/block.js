...
LCARS.element.block = function (oDef) {
  //Begin Required
  this.data = {
    type: "block",
...
LCARS.element.block.prototype = {
  //Required.  Create DOM element and base class
  create: function (oDef) {
    if (oDef.href !== undefined) {
      var element = $('<a id="' + this.data.id + '" class="block"></a>');
    } else {
      var element = $('<div id="' + this.data.id + '" class="block"></div>');
    }
    return element;
  },
