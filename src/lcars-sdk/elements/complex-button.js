...
LCARS.element.complexButton = function (oDef) {
  ...
  this.data = {
    type: "complexButton",
    id: oDef.id || "complexButtonSID" + Math.random().toString(36).substr(2, 9),
  };
...
LCARS.element.complexButton.prototype = {
  //Required.  Create DOM element and base class
  create: function (oDef) {
    if (oDef.href !== undefined) {
      var element = $(
        '<a id="' + this.data.id + '" class="complex-button"></a>'
      );
    } else {
      var element = $(
        '<div id="' + this.data.id + '" class="complex-button"></div>'
      );
    }
    return element;
  },
